...
int getHighestScenicScore2(vector<vector<int>> trees)
{
...
int getHighestScenicScore(vector<vector<int>> trees)
{
    ...
    {
        vector<int> row = trees[r];
        vector<pair<int, pair<int, int>>> peaksRow;
        vector<pair<int, pair<int, int>>> peaksCol;
        ...
        for (int c = 1; c < row.size(); c++)
        {
            ...
            if (heightRow <= row[c - 1])
            {
                peaksRow.push_back(make_pair(row[c - 1], make_pair(r, c - 1)));
                // cout << "Anew peak: " << r << ", " << c - 1 << " : " << row[c - 1] << endl;
            }
            // Larger than last. Check which "peak" blocks the view
            else
            {
                bool hasFound = false;
                vector<pair<int, pair<int, int>>>::reverse_iterator it;
                for (it = peaksRow.rbegin(); it != peaksRow.rend(); it++)
                {
                    // found the blocking tree. Add distance between them
                    if (it->first >= heightRow)
                    {
            ...
            if (heightCol <= trees[c - 1][r])
            {
                peaksCol.push_back(make_pair(trees[c - 1][r], make_pair(c - 1, r)));
                cout << "Bnew peak: " << c - 1 << ", " << r << " : " << trees[c - 1][r] << endl;
            }
            // Larger than last. Check which "peak" blocks the view
            else
            {
                bool hasFound = false;
                vector<pair<int, pair<int, int>>>::reverse_iterator it;
                for (it = peaksCol.rbegin(); it != peaksCol.rend(); it++)
                {
                    // found the blocking tree. Add distance between them
                    if (it->first >= heightCol)
                    {
        ...
        for (int c = row.size() - 2; c >= 0; c--)
        {
            ...
            if (heightRow <= row[c + 1])
            {
                peaksCol.push_back(make_pair(row[c + 1], make_pair(c + 1, r)));
                // cout << "Cnew peak: " << r << ", " << c + 1 << " : " << row[c + 1] << endl;
            }
            // Larger than last. Check which "peak" blocks the view
            else
